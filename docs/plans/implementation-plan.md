...
- DOM Capture
- Accessibility Tree Capture Adapter。`chrome.debugger` permissionを使う場合は任意追加タスクとして扱う。
- ChangeRecord UI
- TraceLink UI
- 影響候補表示
...
- TestStep.targetUiNodeIdによるTestCase手順とUiNodeの関連づけ

P0bでは、`TestStep.targetUiNodeId` から導ける `testCase depends_on uiNode` TraceLinkは永続化しない。詳細は `docs/specs/06-traceability-spec.md` を正とする。
...
- UI解析はDOM構造のみを正本とせず、DOM CaptureとAccessibility Tree Captureを統合したHybrid UI Captureとして扱う。
- Accessibility Tree由来のrole、accessible name、stateはUiNode候補の補助情報であり、ユーザー確認なしに仕様正本へ昇格しない。
...
| Accessibility Tree、Hybrid UI Capture、UiCaptureCandidate | `docs/specs/11-accessibility-tree-capture.md` |
...
| 6     | Element Picker / UI Capture | 対象UI要素をUiNode候補として取り込む。AX取得は任意追加タスクで扱う                    | `docs/specs/04-chrome-extension-spec.md`, `docs/specs/11-accessibility-tree-capture.md`, `docs/specs/01-domain-model.md` |
...
- DOM情報を `UiCaptureCandidate` としてレビューできる。
- `UiCaptureBundle` をWebアプリ側へ取り込める。
- Accessibility Tree Capture Adapterを実装する場合は、AX取得失敗時もDOM Captureで継続できる。
