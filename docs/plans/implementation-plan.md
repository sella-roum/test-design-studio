...
4. `docs/plans/task-breakdown.md`
   - PR単位のタスク一覧。
5. `docs/plans/tasks/*.md`
   - 各PRの詳細なGoal、Scope、Non-goals、Acceptance criteria。
6. `docs/specs/*.md`
   - 実装判断に使う仕様の正本。
7. `docs/design.md`
   - 長期構想と背景思想。
...
- ただし、Phase 1、Phase 2のように基盤構築が目的のPRでは、タスクのAcceptance criteriaを優先する。
...
| 初期実装でやらないこと | `docs/specs/09-non-goals.md` |

## フェーズ

| Phase | 目的 | 主な成果物 | 主な参照 |
|---|---|---|---|
| 0 | 実装準備 | 実装計画、タスク分解、AI指示、仕様分割 | `AGENTS.md`, `docs/specs/*`, `docs/plans/*` |
| 1 | アプリ基盤 | Vite、React、TypeScript、品質ゲート、Dexie初期化 | `docs/specs/02-storage-design.md` |
| 2 | ドメイン・保存層 | ProjectからChangeRecordまでの型、Repository、DB schema | `docs/specs/01-domain-model.md`, `docs/specs/02-storage-design.md` |
| 3 | Web編集フロー | Project、Feature、Screen、UiNode、Data、Rule、Viewpoint、Caseの手動編集 | `docs/specs/03-web-app-spec.md` |
| 4 | Export / Import | Markdown、JSON、CSV、schemaVersion、復元 | `docs/specs/05-import-export-spec.md` |
...
| 7 | 変更管理・影響追跡 | ChangeRecord、TraceLink、影響候補表示 | `docs/specs/06-traceability-spec.md`, `docs/specs/07-change-management-spec.md` |
...
- Webアプリ単体でProject、Feature、Screen、UiNode、DataType、BusinessRule、TestViewpoint、TestCaseを作成・編集できる。
