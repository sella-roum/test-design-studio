...
- `docs/specs/ui/01-ui-design-system.md`
- `docs/specs/ui/02-app-shell-navigation.md`
- `docs/specs/ui/04-feature-workspace-ui-spec.md`
- `docs/specs/ui/07-ui-implementation-order.md`
...
- UI実装では `docs/specs/ui/` 配下の対象仕様を確認し、画面構成、空状態、非対象範囲を揃える。
...
- `docs/specs/ui/02-app-shell-navigation.md`
- `docs/specs/ui/03-project-dashboard-ui-spec.md`
...
- Projectを論理削除できる。
- Projectが存在しない場合、次に取るべき行動が分かる空状態を表示できる。
- 再読み込み後もProjectが残る。
...
- Project DashboardまたはFeature一覧で、Feature Workspaceへ進む導線が分かる。
...
- Feature HeaderでProject / Featureの現在位置が分かる。
- セクションを切り替えられる。
- Featureの基本情報が表示される。
- Open Questionsセクションの枠が存在する。
- 各セクションに空状態または準備中表示がある。
...
- 選択中のScreen / UiNode詳細を確認できるパネルを追加する。
...
- Screen / UiNodeが未作成の場合、次の作成導線が分かる空状態を表示できる。
- 再読み込み後もツリーが保持される。
...
- DataType / BusinessRuleが未作成の場合、次の作成導線が分かる空状態を表示できる。
- 登録内容が再読み込み後も保持される。
...
- `docs/specs/ui/04-feature-workspace-ui-spec.md`
- `docs/specs/ui/05-test-design-editor-ui-spec.md`
- `docs/specs/ui/06-export-traceability-ui-spec.md`
