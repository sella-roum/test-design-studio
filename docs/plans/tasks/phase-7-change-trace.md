...
- `UiCaptureCandidate` は一時候補であり、TraceLinkやChangeRecordの正本targetにはしない。
- 影響候補は自動確定しない。
- AIによる影響分析は初期実装しない。
...
- GitHub PR連携は実装しない。
- DOM差分自動検出は実装しない。
- Accessibility Tree差分自動検出は実装しない。
- AIによる変更要約は実装しない。
- UiCaptureCandidateをChangeRecord targetにしない。
...
- accessible name / role / state変更時の自動化影響候補を表示する。
- 候補を採用・却下できるようにする。
...
- Accessibility Tree差分の完全自動検出は実装しない。

### Acceptance criteria

- ChangeRecordに対して影響候補を表示できる。
- OpenQuestion更新時の影響候補を表示できる。
- selector変更、accessible name変更、role変更、state変更の影響候補を扱える。
- 候補をユーザーが採用・却下できる。
...
- UiNode変更: TestStep.targetUiNodeIdで参照しているTestCase、または関連TestViewpointを候補にする。
...
- selector変更: automationSuitabilityが高いTestCase、または該当UiNodeを参照するTestStepを持つTestCaseを候補にする。
- accessible name / role / state変更: `locatorStrategy: "role"`、`locatorHint`、またはautomationSuitabilityが高いTestCaseを候補にする。
...
## UiCaptureCandidate関連の扱い

Chrome拡張候補とUiNodeの関係は、TraceLinkではなく候補レビューとChangeRecordで扱う。

方針:

- `UiCaptureCandidate` はTraceNodeTypeに含めない。
- 新規UiNodeがUiCaptureCandidateから作られた場合も、`uiNode derived_from uiCaptureCandidate` は作成しない。
- 候補由来であることを残したい場合は、UiNodeの補助メタ情報、またはChangeRecordのbefore/after/reasonで扱う。
- 既存UiNodeへの更新候補として扱う場合も、候補そのものではなく更新後のUiNodeにChangeRecordを作る。

この方針は、DOM CaptureやAccessibility Tree Captureの取得結果を仕様正本として扱わないためである。
