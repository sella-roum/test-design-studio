...
- `UiCaptureCandidate` はP1の候補モデルであり、Phase 2のP0必須Repositoryには含めない。
...
- selectorHint、role、componentType、textHint、accessibleNameHint、locatorStrategy、locatorHint、descriptionなど、Chrome拡張からの取り込みと将来のPlaywright補助に必要な基本属性を保持できるようにする。
...
- selectorHint、accessibleNameHint、locatorHintを保存できる。
- Repositoryテストが通る。
...
- カバレッジ可視化は実装しない。
- AIによる関連推定は実装しない。
- UiCaptureCandidateはこのタスクでは実装しない。
