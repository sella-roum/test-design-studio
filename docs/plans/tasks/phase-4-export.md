...
- `UiCaptureBundle` の形式方針は本書にも記載するが、実装はPhase 5-6 / TASK-021で扱う。TASK-018ではProject `ExportBundle` のみを対象にする。
...
この節はImport / Export全体仕様として `UiCaptureBundle` の形式方針を記録するためのものである。実装はPhase 5-6 / TASK-021で扱い、TASK-018のScopeには含めない。
...
- TASK-018のimport対象はProject単位の `ExportBundle` のみとする。
- `UiCaptureBundle` importはImport / Export全体仕様としては許容するが、実装はPhase 5-6 / TASK-021で扱う。
- 既存Projectに上書きしない。
