...
Chrome拡張は、テスト対象アプリケーションを開いた状態で、現在の画面情報や選択したUI要素を Test Design Studio に取り込むための入力補助である。

Chrome拡張は仕様の正本ではない。取得したDOM情報やAccessibility Tree情報は候補として扱い、利用者が確認・編集してからUiNodeやChangeRecordに反映する。

UI解析はDOM構造のみを正本としない。DOM Captureはselector候補、属性、技術的識別子を取得するために使い、Accessibility Tree Captureはrole、accessible name、description、状態などのユーザー視点の意味情報を取得するために使う。両者を統合した候補は `UiCaptureCandidate` として扱う。

詳細は `docs/specs/11-accessibility-tree-capture.md` を参照する。
...
- Accessibility Capture Adapter: 必要なタスクで明示された場合のみ、CDP Accessibility domain等を使ってAX情報を取得する任意コンポーネント。
...
5. `chrome.debugger` permission は標準MVPに含めない。Accessibility Tree Capture Adapterを実装するタスクで明示された場合のみ追加する。

### Accessibility Tree permissions

Accessibility TreeをChrome拡張内で取得する場合、`chrome.debugger` API経由でChrome DevTools ProtocolのAccessibility domainを利用する可能性がある。

`debugger` permission は強い権限であるため、次を必須とする。

- 該当タスクで明示されていること。
- PR本文に必要理由、利用範囲、取得データ制限、detach方針を書くこと。
- AX取得に失敗してもDOM Captureで最低限動作すること。
- AX情報を仕様正本として自動登録しないこと。
...
- ページ全体のDOMまたはAX treeを永続スナップショットとして保存しない。
...
- `text`、`accessibleName`、`description`、`placeholder`、`ariaLabel` は最大100文字に切り詰める。
...
- 取得した候補を `UiCaptureCandidate` として保存できる。
- `UiCaptureBundle` export/importを通じてWebアプリ本体と連携できる。
...
Accessibility Tree取得はContent Scriptだけでは完結しない可能性があるため、該当タスクで明示された場合のみAccessibility Capture Adapterで扱う。
...
### Capture accessibility node

該当タスクで明示された場合のみ、選択要素に対応するAccessibility Tree情報を取得する。
...
AX由来の `name` / `description` もredaction rulesとtext length limitの対象にする。
...
`UiCaptureBundle` はWebアプリ側でimportし、候補レビュー画面に取り込む。Projectの完全復元や既存Project置き換えには使わない。
...
- Accessibility Tree取得は `TASK-021A` などの明示タスクでのみ追加する。
...
1. 拡張側で候補を `UiCaptureBundle` としてJSON exportする。
2. Webアプリ側で `UiCaptureBundle` をimportする。
3. Webアプリ側の候補レビュー画面で `UiCaptureCandidate` を確認する。
...
- DOMやAccessibility Treeから完全な仕様書を自動生成する。
...
- Accessibility Tree取得を実装する場合は、AX node正規化、redaction、DOM/AX対応付けを単体テストする。
