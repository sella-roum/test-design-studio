...
| Card | 内容 | P0 |
|---|---|---:|
| Coverage | TraceLinkまたはViewpoint/Case紐づき状況 | 簡易 |
...
```text
coverage = source elementにderived_from TraceLinkを持つTestViewpoint数 / 全TestViewpoint数
```

TraceLink UIが未実装の場合は、Coverage cardをplaceholderにするか非表示にしてよい。
...
| Traceability | 関連性確認 | 簡易 | 本格UIはPhase 7 |
...
|---|---|
| name | 画面名 |
| screenType | page / modal / drawerなど |
...
| Base type | string / number / date / enum |
...
| ID | BR-001など |
| Name | ルール名 |
| Rule type | validation / permission / display / transition / errorなど |
| Condition | 条件 |
| Result | 結果 |
| Related UI | 関連UiNode |
...
Matrix UIは `06-export-traceability-ui-spec.md` とPhase 7を正とする。
...
| Traceability | 関連データが不足していることを説明 |
...
- P0対象外のTraceability / Changes / Export Previewは準備中または簡易表示に留められる。
