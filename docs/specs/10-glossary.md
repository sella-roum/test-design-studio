...
対象ページのDOM構造、属性、selector候補、近傍ラベルなどを取得するUI候補取得方式。仕様の正本ではない。

## Accessibility Tree

ブラウザが支援技術向けに構築するUI意味ツリー。DOM構造そのものではなく、role、accessible name、description、stateなど、ユーザー視点の操作対象に近い情報を持つ。

## Accessibility Tree Capture

Accessibility Treeからrole、accessible name、description、checked / selected / expanded / disabledなどの状態を取得するUI候補取得方式。仕様の正本ではない。

## Accessible Name

支援技術やPlaywrightのrole locatorで使われる要素名。ラベル、aria-label、要素テキストなどから計算される。

## AX Node

Accessibility Tree上のノード。DOM nodeと対応付く場合もあるが、必ず1対1対応するとは限らない。

## Hybrid UI Capture

DOM CaptureとAccessibility Tree Captureを統合し、selector候補とユーザー視点のrole/name/stateを併せて `UiNode` 候補化する方式。

## UiCaptureCandidate

Chrome拡張やPlaywrightなどから取得したUI情報の候補。DOM由来情報とAccessibility Tree由来情報を保持できる。仕様の正本ではなく、ユーザーが確認・編集してからUiNodeへ変換する。

## UiCaptureBundle

`UiCaptureCandidate` をWebアプリ側へ取り込むためのJSON形式。Project完全バックアップであるExportBundleとは別形式として扱う。

## DomCaptureCandidate

旧称。DOM情報だけを前提にした候補名。新規実装では `UiCaptureCandidate` に寄せる。
...
Chrome拡張、UiCaptureCandidate、ChangeRecord、TraceLink UI、影響候補表示を追加する段階。
