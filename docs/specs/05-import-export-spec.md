...
| 対象 | 置き換える参照 |
|---|---|
| Feature.id | Screen.featureId, BusinessRule.featureId, OpenQuestion.featureId, TestViewpoint.featureId, TestCase.featureId, Flow.featureId |
| Screen.id | UiNode.screenId, BusinessRule.screenId, OpenQuestion.screenId, State.screenId, Flow.startScreenId, Flow.endScreenId, FlowStep.screenId |
| UiNode.id | UiNode.parentId, BusinessRule.uiNodeId, OpenQuestion.uiNodeId, TestStep.targetUiNodeId, State.uiNodeId, FlowStep.uiNodeId |
...
| 任意の対象ID | TraceLink.fromId, TraceLink.toId, ChangeRecord.targetId, OpenQuestion.relatedId |
