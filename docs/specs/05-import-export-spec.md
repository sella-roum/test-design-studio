...
```text
UiCaptureBundle JSON: Chrome拡張候補取り込み用
CSV: TestCase一覧の外部利用用
```

`DomCaptureBundle JSON` は旧称として扱う。新規実装では `UiCaptureBundle JSON` に寄せる。
...
Reserved model fieldにデータが含まれている場合、P0では取り込まず、import前確認で警告を表示したうえで、P0必須モデルのimportは継続してよい。
...
次のいずれかに該当する `traceLinks` / `changeRecords` は、P0では取り込まない。
...
P0未対応のTraceLink / ChangeRecordが含まれる場合は、該当するTraceLink / ChangeRecordを除外し、import前確認で警告を表示する。

P0必須モデル本体の参照整合性が壊れている場合はimportを失敗させる。例えば、`TestCase.viewpointId` が取り込み対象の `testViewpoints` に存在しない、`UiNode.screenId` が取り込み対象の `screens` に存在しない、といった場合は部分取り込みしない。
...
- traceLinks / changeRecords がP0 storage-level TraceNodeTypeだけを参照しているかを確認する。
- traceLinks / changeRecords の参照先IDが、P0 importerで取り込まれるデータ内に存在するかを確認する。
- P0必須モデル本体の参照IDに整合性がある。
...
新規Projectとして取り込む場合、Project IDを含むすべてのP0対象レコードIDを常に再生成する。

IDを維持する方式は採用しない。同じexport fileを複数回importした場合でも、各import結果が独立したProjectとして扱えるようにするためである。

関連レコードのprojectIdは新しいProject IDへ置き換える。

P0対象レコードIDを再生成した後、参照IDは新旧ID対応表に基づいて同一transaction内で置き換える。
...
TraceLink / ChangeRecordの再マッピングは、P0 importerで取り込む対象に参照先が存在する場合だけ行う。参照先がReserved model、またはP0取り込み対象外のTraceNodeTypeである場合、該当するTraceLink / ChangeRecordは取り込まず、警告対象にする。
...
### UiCaptureBundle import requirements

- `exportType: "ui-capture"` を検証する。
- candidates配列を検証する。
- Project全体の置き換えや復元には使わない。
- 取り込んだ候補は、候補レビュー画面でユーザーが確認する。
- `UiCaptureCandidate` は自動で `UiNode` にしない。
- ユーザーが確認・編集・承認した候補だけを `UiNode` へ変換する。
- 取り込み時にもData safety / Privacyのredaction rulesを再確認する。
- Accessibility Tree由来のrole/name/stateも仕様正本として自動採用しない。

### Legacy DomCaptureBundle compatibility

`DomCaptureBundle` は旧称として扱う。既存ファイルを取り込む必要がある場合は、`exportType: "dom-capture"` を `UiCaptureBundle` 相当へ変換する互換処理を検討してよい。

ただし、新規exportでは `exportType: "ui-capture"` を使う。
...
test-design-studio-ui-capture-{yyyyMMdd-HHmmss}.json
```
...
- P0未対応TraceNodeTypeを参照するTraceLink / ChangeRecordの永続化
- CSV export
- GitHubへの直接push
