...
TraceLinkはP0でも保存モデルとして存在するが、専用の高度なTraceability UIはPhase 7を正とする。P0ではTestCase作成時のViewpoint紐づけ、または簡易的な関連付けに留めてよい。

ChangeRecord、影響候補表示、Chrome拡張、Export / Import は長期ワークフローには含まれるが、それぞれ Phase 4、Phase 5-7 の詳細タスクを正とする。
...
Phase 3では `Overview`、`Screens / UI Tree`、`Data / Rules`、`Open Questions`、`Viewpoints`、`Test Cases` を優先する。`Changes`、`Traceability`、`Export Preview` は枠だけ用意してもよいが、実装判断は Phase 4 / Phase 7 のタスクを優先する。
...
- Feature、Screen、UiNode、またはTraceNodeTypeに対応する対象へ紐づけられる。
