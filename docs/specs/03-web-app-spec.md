...
Phase 3 の初期Webアプリ実装では、次の手入力フローを対象にする。
...
TraceLink、ChangeRecord、Export / Import は長期ワークフローには含まれるが、それぞれ Phase 4、Phase 7 の詳細タスクを正とする。
...
Phase 3では `Overview`、`Screens / UI Tree`、`Data / Rules`、`Viewpoints`、`Test Cases` を優先する。`Changes`、`Traceability`、`Export Preview` は枠だけ用意してもよいが、実装判断は Phase 4 / Phase 7 のタスクを優先する。
